...
try:
    while 1:
        ...
        while 1:
            ...
            query = "UPDATE meta_db SET edgeSite = '" + EDGE_SITE + "' WHERE datasetID = '" + userDataset + "'";
            print (query)
            cur.execute(query)
            updated_rows = cur.rowcount
            con.commit()
            
            query = "UPDATE meta_db SET clusterLocation = '" + CLUSTER + "' WHERE datasetID = '" + userDataset + "'";
            print (query)
            cur.execute(query)
            updated_rows = cur.rowcount
            con.commit()
